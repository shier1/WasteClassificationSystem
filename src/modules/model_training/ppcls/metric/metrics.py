...
class WordTreeTopkAcc(nn.Layer):
    ...
    def forward(self, x, label):
        ...
        classes_label = label[:, :4]
        names_label = label[:,4:]

        class_int = paddle.argmax(classes_label, axis=1, keepdim=True)
        names_int = paddle.argmax(names_label, axis=1, keepdim=True)

        for k in self.topk:
            if k == 1:
                metric_dict["level1 top1"] = paddle.metric.accuracy(
                    x[:, :4], class_int, k=1
                )
            metric_dict["level2 top{}".format(k)] = paddle.metric.accuracy(
                x[:, 4:], names_int, k=k)
        return metric_dict
