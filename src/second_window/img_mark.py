...
class mymainwindow(QWidget,Ui_Form):
    ...
    def saveFile(self):
        ...
        os.makedirs(os.path.join(self.path, 'image').replace('\\', '/'), exist_ok=True)    #创建存放原图的文件夹，用makedirs函数
        os.makedirs(os.path.join(self.path, 'label').replace('\\', '/'), exist_ok=True)   #创建存放json文件的文件夹

    def openimage(self):    #读取文件图片
        self.imgName, self.imgType = QFileDialog.getOpenFileName(self, "导入照片", "", "*.jpg;;*.png;;All Files(*)") #导入图片路径
